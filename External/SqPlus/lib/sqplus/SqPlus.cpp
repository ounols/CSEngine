...
static int setVar(StackHandler & sa,VarRef * vr,void * data) {
  ...
  switch (vr->m_type) {
  case (ScriptVarType) TypeInfo<INT>::TypeID: {
  ...
  case (ScriptVarType) TypeInfo<unsigned>::TypeID: {
  ...
  case (ScriptVarType) TypeInfo<FLOAT>::TypeID: {
  ...
  case (ScriptVarType) TypeInfo<bool>::TypeID: {
  ...
  case (ScriptVarType) TypeInfo<SQUserPointer>::TypeID: {
    const SQChar * el = sa.GetString(2);
    SCSNPRINTF(g_msg_throw.s,sizeof(g_msg_throw),_SC("setVar(): Cannot write to an SQUserPointer: %s"),el);
    throw SquirrelError(g_msg_throw.s);
  } // case
  case (ScriptVarType) TypeInfo<ScriptStringVarBase>::TypeID: {
...
static int getVar(StackHandler & sa,VarRef * vr,void * data) {
  ...
  case (ScriptVarType) TypeInfo<const SQChar *>::TypeID: {
    if (!(vr->m_access & VAR_ACCESS_CONSTANT)) {
