...
void DAELoader::Load(const char* path, LOAD_TYPE type) {
	...
	if (type == MESH || type == ALL || type == AUTO) {
		try {
			//
...
void DAELoader::LoadSkin(const XNode& root_s) {
...
void DAELoader::LoadSkeleton(const XNode& root_s) {
...
void DAELoader::LoadGeometry(const XNode& root_g, DAEMeshData* meshData) {
...
void DAELoader::ReadPositions(const XNode& data, std::vector<VertexSkinData*> vertexWeight,
                              DAEMeshData* meshData) const {
...
void DAELoader::ConvertDataToVectors(DAEMeshData* meshData) {
	...
	unsigned int vertices_size = meshData->vertices.size();
...
DAELoader::AttachDataToObjSurface(int vertices_size, std::vector<float> vertices, std::vector<float> normals,
	...
	unsigned int sizeIndices = indices.size() / 3;
...
SPrefab* DAELoader::GeneratePrefab(Animation* animation, SPrefab* prefab) {
	...
	JointComponent* joint_root_component = m_isSkinning
		                                       ? joint_root->GetChildren().front()->GetComponent<JointComponent>()
		                                       : nullptr;
...
SPrefab* DAELoader::GeneratePrefab(const char* path, Skeleton* skeleton, MeshSurface* mesh, Animation* animation,
	...
	for (const auto& meshElement : meshData) {
		meshElement->meshSurface->LinkResource(prefab_id + "." + meshElement->meshName + "?meshElement");
	}
