...
std::string SSceneLoader::ComparePrefab(SGameObject* obj) {
    ...
    result += std::string("<prefab name=\"") + ConvertSpaceStr(obj->GetName())
    ...
    for (const auto& object_node: n_o->children) {
        ...
        if (!isChanged) {
            ...
            for (const auto& obj_node: n_o->children) {
                if (obj_node.getAttribute("hash").value == parent_o) {
                    o_parent_resID = obj_node.getAttribute("ref").value;
                    break;
                }
            }

            for (const auto& prf_node: n_p->children) {
                if (prf_node.getAttribute("hash").value == parent_p) {
                    p_parent_resID = prf_node.getAttribute("ref").value;
                    break;
                }
            }
...
std::string SSceneLoader::PrintGameObject(SGameObject* obj) {
    std::string values;
    const auto& objRef = dynamic_cast<SGameObjectFromSPrefab*>(obj);

    // Pre-allocate space for the string to avoid costly re-allocations
    values.reserve(1024);

    const std::string& res_id = obj->GetResourceID();
    values += "<gameobject name=\"" + ConvertSpaceStr(obj->GetName())
              + "\" hash=\"" + obj->GetHash() + "\"";
    if (!res_id.empty()) {
        values += " fileid=\"" + res_id + "\"";
    }
    if (objRef != nullptr) {
        values += " ref=\"" + objRef->GetRefHash() + "\"";
    }
    values += ">\n";
