    ...
    for (auto comp : comps) {
        ...
        for (auto value : node.children) {
            ...
            for (int i = 0; i < v_values.size(); i++) {
                v_values[i] = ConvertSpaceStr(v_values[i]);
            }
...
void SSceneLoader::LinkingResourceID(XNode node, SGameObject* root, std::vector<SSceneLoader::NodeKey*>& objs,
                                     std::vector<SSceneLoader::ComponentValue*>& comps) {
