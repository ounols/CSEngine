...
void DrawableSkinnedMeshComponent::CopyReference(const SComponent& src, const std::map<SGameObject*, SGameObject*>& lists_obj,
                                                 const std::map<SComponent*, SComponent*>& lists_comp) {
    if (&src == nullptr) return;
    const auto& convert = static_cast<const DrawableSkinnedMeshComponent*>(&src);
