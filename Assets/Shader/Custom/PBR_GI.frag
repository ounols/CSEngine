...
//[texture.sdfmap]//
uniform sampler2D u_sampler_sdf;
...
vec4 GiLinear(vec3 uvw, vec3 pos) {
	float texId;
	vec2 st;
	vec3 ipos = floor(pos);
	...
	index = max(index, 0.);
//	if(index < 0. || index > index_pos_y) return vec4(1., 0, 0, 1);

    {
        ...
        float u = (phi + 3.14159265359) / (2.0 * 3.14159265359);// [0, 1] 범위로 변환
        float v = theta / 3.14159265359;
        st = vec2(u, v);
    }

    st = vec2(st.x / u_cell_size.x, st.y / u_cell_size.y);
    vec4 color = texture(u_sampler_sdf, st + vec2(
        floor(mod(index, u_cell_size.x)) * inv_cell_size.x,
        floor(index / u_cell_size.x) * inv_cell_size.y
    ));
    return color;
}

vec4 GiSmooth(vec3 uvw, vec3 pos) {
    ...
    vec4 c000 = GiLinear(direction, pos);
    vec4 c100 = GiLinear(direction, pos + offset_x);
    vec4 c010 = GiLinear(direction, pos + offset_y);
    vec4 c110 = GiLinear(direction, pos + offset_x + offset_y);
    vec4 c001 = GiLinear(direction, pos + offset_z);
    vec4 c101 = GiLinear(direction, pos + offset_x + offset_z);
    vec4 c011 = GiLinear(direction, pos + offset_y + offset_z);
    vec4 c111 = GiLinear(direction, pos + offset_x + offset_y + offset_z);
...
void main(void) {
    ...
    lowp float metallic  = texture(u_sampler_metallic, v_textureCoordOut).r;
    lowp float roughness = texture(u_sampler_roughness, v_textureCoordOut).r * 0.7;
    lowp float ao        = 1.f;
	...
	vec3 SDF_P = (vec3(1, 0, 0) + u_node_size * u_node_space * 0.5) / u_node_space;
    ...
    vec3 gi_i = GiSmooth(N, SDF_P).rgb;

    vec3 diffuse      = (gi_i) * albedo;
    ...
    vec3 gi_s = GiSmooth(R, SDF_P).rgb * (kS * brdf.x + brdf.y);
    gi_s = max(vec3(0.), gi_s);
    gi_s = min(vec3(1.), gi_s);

    vec3 ambient = (kD * diffuse + gi_s) * ao;
