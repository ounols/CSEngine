...
//	if(index < 0. || index > index_pos_y) return vec4(1., 0, 0, 1);
	...
	vec4 color = vec4(0);
	for (int i = 0; i < 6; ++i) {
		vec4 side = texture(u_sampler_sdf, st + vec2(float(i) * (1./6.), index * (1./index_pos_y)));
...
void main(void) {
	...
	lowp vec3 albedo     = vec3(0.3);
	lowp float metallic  = 0.99;
	lowp float roughness = 0.05;
	lowp float ao        = 1.f;
	...
	vec3 diffuse      = (irradiance + gi_i) * albedo;
